...
if ($method == 'fetch_pending') {
	...
	$query = "SELECT * FROM borrow_list WHERE facility LIKE '$facility_name%' AND status = 'Pending'";
...
if ($method == 'disapproved') {
	...
	$query = "UPDATE borrow_list SET status = 'Dis-Approved' WHERE id = '$id'";
	$stmt = $conn->prepare($query);
	if ($stmt->execute()) {
		echo 'success';
	}else{
...
if ($method == 'approved') {
	...
	$query = "UPDATE borrow_list SET status = 'Approved' WHERE id = '$id'";
	$stmt = $conn->prepare($query);
	if ($stmt->execute()) {
		$stmt = NULL;

		$query = "UPDATE facilities SET status = 'Not_Available', date_updated = '$server_date_only' WHERE facility = '$facility'";
		$stmt = $conn->prepare($query);
		if ($stmt->execute()) {
			$stmt = NULL;
